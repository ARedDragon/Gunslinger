...
function pushGlobalChat(item) {
  globalChat.push(item);
  if (globalChat.length > MAX_CHAT_MESSAGES) {
    globalChat.shift();
  }
}

function broadcastGlobalChat(item) {
  const payload = JSON.stringify({ type: 'global_chat', item });
...
server.on('upgrade', (request, socket, head) => {
  ...
  wss.handleUpgrade(request, socket, head, (ws) => {
    ...
    sendWs(ws, { type: 'global_chat_history', items: globalChat });
